...
SitePrev <- function(input_csv_seq){
  ...
  for (i in 1:(length(seq[1,])-4)){
    ...
    for (j in year){
      ...
      data <- data[which(month(data$date) < 3 | month(data$date) > 9),]
...
# write.csv(na_prev,file="./toydata_result/H3N2_NA_HK_prevalence.csv",row.names = F)
...
ha_gmeasure <- Getgmeature(input_prev = ha_prev, theta_range = c(5:9),h_range = c(0:9))
na_gmeasure <- Getgmeature(input_prev = na_prev, theta_range = c(5:9),h_range = c(0:9))

# please note that for coding convenience, the theta_range here is the actual theta*10. For example, theta_range = c(5:9) means theta from 0.5 to 0.9.
...
HA_EMsite <- EffMutation(input_prev_table = ha_prev, theta = 0.5)
...
# The output predicted consensus sequence are "BethH3_HA_predicted consensus seq.csv" and "BethH3_NA_predicted consensus seq.csv"
# The output predicted EM set is summarized in "HKH3_predicted_EMset.csv"


# Step 6.2: The function WTselection allows to select wildtype strain integrating mutation information on both HA and NA (two-protein)
...
WTselection <- function(input_ha_seq, input_na_seq, pred_emset, ha_pre_seq, na_pre_seq, ha_beta, na_beta, ha_as, na_as, pre_year){
  ...
  for (s in pre_year){
    ...
    total_weighted_EMD <- c()
    EMD_ha <- c()
    EMD_na <- c()
    ha_ve_site <- intersect(ha_em,h3ha_as_ab) # H1N1: EMs on antigenic sites, H3N2: EMs on antigenic sites A & B
    na_ve_site <- na_as # H1N1: EMs on antigenic sites, H3N2: antigenic sites 
    ...
    for (k in 1:length(ha_test[,1])){ # genetic distance in dynamic predictor set
      ...
      total_weighted_EMD <- c(total_weighted_EMD,ve_emd)
      EMD_ha <- c(EMD_ha,ha_emd)
      EMD_na <- c(EMD_na,na_emd)
    } #select the wildtype sequence by minimizing the weighted distance
    
    position <- which(total_weighted_EMD == min(total_weighted_EMD))
...
h3n2_wildtype_result <- WTselection(input_ha_seq = ha_seq, input_na_seq = na_seq, pred_emset = read.csv("./toydata_result/HKH3_predicted_EMset.csv"),
                                    ...
                                    ha_beta = 11, na_beta = 20.2, ha_as <- h3ha_as, na_as <- h3na_as, pre_year <- c(2012:2020))
...
# output: wildtype vaccine strains (two-protein)


# Step 6.3 (optional): The function WTselection_single allows to select wildtype strain on single gene (single protein)

WTselection_single <- function(input_seq, pred_emset, pre_seq, antigenic_site, pre_codon_site, aa_length, pre_year){
  ha_seq <- input_seq
  wildtype <- matrix(ncol=length(ha_seq[1,]),nrow=0)
  colnames(pre_seq)[1] <- "season"
  
  for (s in pre_year){
    print(s)
    ha_test <- ha_seq[which(ha_seq$date<make_date(s-1,3,1)),] 
    pre_hatest <- pre_seq[which(pre_seq$season==s),]
    ha_em <- unique(pred_emset[which(pred_emset[,2]<=s),1])
    total_EMD <- c()
    ha_ve_site <- intersect(ha_em,pre_codon_site)
    ...
    } #select the wildtype sequence based on genetic distance on predictor codon sites
    
    position <- which(total_EMD == min(total_EMD))
    ha_test_second <- ha_test[position,]
    total_antigenic_dis <- c()
    ha_site <- setdiff(union(ha_em,antigenic_site),ha_ve_site) 
    ...
    ha_test_third <- ha_test_second[position,]
    total_hamming_dis <- c()
    ha_rest_site <- setdiff(c(1:aa_length),union(ha_site,ha_ve_site)) 
    ...
    position <- which(total_hamming_dis == min(total_hamming_dis))
    final_position <- position[length(position)]
    wildtype <- rbind(wildtype,ha_test_third[final_position,])
  }
  wildtype$season <- pre_year
  return(wildtype)
}

pred_em <- read.csv("./toydata_result/HKH3_predicted_EMset.csv")

h3n2_singleHA_wildtype <- WTselection_single(input_seq = ha_seq, pred_emset = pred_em[,c(1:2)],pre_year <- c(2012:2020),
                                             pre_seq = read.csv("./toydata_result/BethH3_HA_predicted consensus seq.csv"),
                                             antigenic_site = h3ha_as, pre_codon_site = h3ha_as_ab, aa_length = 566)

h3n2_singleNA_wildtype <- WTselection_single(input_seq = na_seq, pred_emset = pred_em[,c(3:4)],pre_year <- c(2012:2020),
                                             pre_seq = read.csv("./toydata_result/BethH3_NA_predicted consensus seq.csv"),
                                             antigenic_site = h3na_as, pre_codon_site = h3na_as, aa_length = 469)

# write.csv(h3n2_singleHA_wildtype,file="./toydata_result/BethH3_wildtype_singleHA.csv",row.names = F)
# write.csv(h3n2_singleNA_wildtype,file="./toydata_result/BethH3_wildtype_singleNA.csv",row.names = F)

# output: wildtype vaccine strains (single protein)
...
beth_ha_mismatch <- GetMismatch(input_pre_seq = h3n2_wildtype_result$beth_ha, input_cir_seq = ha_seq, test_site = h3ha_epitope, test_year = c(2012:2020))
beth_na_mismatch <- GetMismatch(input_pre_seq = h3n2_wildtype_result$beth_na, input_cir_seq = na_seq, test_site = h3na_epitope, test_year = c(2012:2020))
who_ha_mismatch <- GetMismatch(input_pre_seq = read.csv("H3N2_WHOvaccine_HA.csv"), input_cir_seq = ha_seq, test_site = h3ha_epitope, test_year = c(2012:2020))
who_na_mismatch <- GetMismatch(input_pre_seq = read.csv("H3N2_WHOvaccine_NA.csv"), input_cir_seq = na_seq, test_site = h3na_epitope, test_year = c(2012:2020))

mismatch_result <- cbind(beth_ha_mismatch,beth_na_mismatch,who_ha_mismatch,who_na_mismatch)
colnames(mismatch_result) <- c("beth_ha_mean","beth_ha_sd","beth_na_mean","beth_na_sd","who_ha_mean","who_ha_sd","who_na_mean","who_na_sd")
